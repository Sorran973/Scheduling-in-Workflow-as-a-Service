...
class Drawer(ABC):
    ...
    @abstractmethod
    def draw_cjm_graph(self, nodes, edges):
        pass

    @abstractmethod
    def draw_cjm_gantt(self, nodes):
        pass

    @abstractmethod
    def draw_allocation_result_gantt(self, log, basic, sort):
        pass

    @abstractmethod
    def draw_batches_gantt(self, tasks, folder_name):
        pass
...
class GraphvizDrawer(Drawer):
    ...
    def draw_allocation_result_gantt(self, log):
        pass

    def draw_cjm_graph(self, nodes, edges):

        G = graphviz.Digraph(filename=self.GRAPHVIZ_CJM_GRAPH)
        for arr in nodes:
            for node in arr:
                G.node(str(node.id) + '\n' +
                       node.name + '\n' +
    ...
    def draw_cjm_gantt(self, nodes):
        ...
        plt.show()
        fig.savefig(self.CJM_GANTT, format="pdf")
...
class PyvisDrawer(Drawer):
    ...
    def draw_cjm_graph(self, nodes, edges):
        ...
        network.from_nx(G)
        network.show(self.PYVIS_CJM_GRAPH)
    ...
    def draw_new_cjm_gantt(self, nodes):
    ...
    def draw_batches_gantt(self, tasks, folder_name):
        ...
        plt.show()
        fig.savefig(folder_name, format="pdf")

    def draw_allocation_result_gantt(self, log, basic_name, vm_sort_name):
        color = log[["vm_id"]]
        color = color.drop_duplicates()
        color["color"] = color.apply(lambda row: rand_color(row), axis=1)
        log = pd.merge(log, color, on='vm_id', how='left')

        self.draw_gantt_from_dataframe(log, basic_name)
        log = log.sort_values(["vm_id", "vm_end"])
        self.draw_gantt_from_dataframe(log, vm_sort_name)

    @staticmethod
    def draw_gantt_from_dataframe(log, figure_name):
        # fig, (ax, ax1) = plt.subplots(2, figsize=(16, 6), gridspec_kw={'height_ratios': [6, 1]})
